...
if (!DATAFORSEO_USERNAME || !DATAFORSEO_PASSWORD) {
    throw new Error("DATAFORSEO_USERNAME and DATAFORSEO_PASSWORD environment variables are required");
}
        ...
        if (method === "tools/call" && params?.name) {
            ...
            if (true) {
                ...
                if (apiName.includes("merchant_")) {
                    ...
                    requestData = [
                        Object.fromEntries(Object.entries(merchantParams).filter(([key, value]) => value !== undefined)),
                    ];
                }
                else if (apiName.includes("domain_analytics_") ||
                ...
                if (!apiName.includes("merchant_")) {
                    const otherParams = Object.fromEntries(Object.entries(arguments_).filter(([key, value]) => ![
                        "location_name",
                        "location",
                        "language_code",
                        "language_name",
            ...
            if (arguments_.location_code) {
                ...
                try {
                    ...
                    if (locationsResponse.status === 200 &&
                        ...
                        let matchingLocation = locations.find((loc) => loc.location_name &&
                            loc.location_name
                                .toLowerCase()
                        ...
                        if (!matchingLocation) {
                            ...
                            matchingLocation = locations.find((loc) => loc.location_name &&
                                loc.location_name
                                    .toLowerCase()
                        ...
                        if (!matchingLocation) {
                            const searchTerms = arguments_.location_name
                                ...
                                .split(/[,\s]+/);
                            matchingLocation = locations.find((loc) => loc.location_name &&
                                searchTerms.some((term) => term.length > 2 &&
    ...
    try {
        ...
        if (!hasAllowedPath) {
            return res
                .status(400)
                .json({
                error: "Path not allowed. Allowed paths: /business-data/, /lighthouse/",
            });
        }
            ...
            if (summary_only) {
                const optimizedResponse = {
                    ...
                    pagination: {
                        ...
                        total_pages: Math.ceil((rawData.tasks?.[0]?.result?.[0]?.total_items_count || 0) /
                            per_page),
                        has_next: page <
                            Math.ceil((rawData.tasks?.[0]?.result?.[0]?.total_items_count || 0) /
                                per_page),
                        has_prev: page > 1,
                    },
                    summary: {
                        ...
                        performance_score: rawData.tasks?.[0]?.result?.[0]?.items?.[0]?.performance_score ||
                            null,
                    },
        ...
        if (dataforseoResponse.status === 200) {
            ...
            if (summary_only) {
                const optimizedResponse = {
                    ...
                    preview: {
                        html_preview: rawData.tasks?.[0]?.result?.[0]?.raw_html?.substring(0, 500) ||
                            "N/A",
                        screenshot_url: rawData.tasks?.[0]?.result?.[0]?.screenshot_url || null,
                        content_preview: rawData.tasks?.[0]?.result?.[0]?.content?.substring(0, 1000) ||
                            "N/A",
                    },
                    blob_storage: {
                        storage: "vercel-blob",
...
app.post("/v3/onpage_lighthouse", async (req, res) => {
    try {
        ...
        switch (type) {
            ...
            case "task_get":
                ...
                try {
                    ...
                    return res
                        .status(500)
                        .json({ error: "Summary endpoint error: " + error.message });
                }
            case "summary":
        ...
        const dataforseoResponse = await makeDataForSEORequest(endpoint, requestData, type === "languages" ||
            type === "audits" ||
            type === "versions" ||
            type === "tasks_ready" ||
            type === "task_get"
...
app.post("/v3/keywords_bing_advanced", async (req, res) => {
    try {
        ...
        const dataforseoResponse = await makeDataForSEORequest(endpoint, requestData, type.includes("task_get") ||
