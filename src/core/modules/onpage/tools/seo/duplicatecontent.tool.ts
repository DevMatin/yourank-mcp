...
export class DuplicateContentTool extends BaseTool {
  ...
  getDescription(): string {
    return "Pages with similar content to specified page";
  }
  ...
  async handle(params: { id: string; limit?: number; offset?: number; filters?: any[]; order_by?: any[] }): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/on_page/duplicate_content', 'POST', [requestData]);
