...
interface GoogleTrendsExploreParams {
  ...
  date_from?: string;
  date_to?: string;
  time_range?: 'past_hour' | 'past_4_hours' | 'past_day' | 'past_7_days' | 'past_30_days' | 'past_90_days' | 'past_12_months' | 'past_5_years';
  item_types?: ('google_trends_graph' | 'google_trends_map' | 'google_trends_topics_list' | 'google_trends_queries_list')[];
  category_code?: number | null;
}

interface GoogleTrendsExploreRequest {
  location_name?: string | null;
  language_code?: string | null;
  keywords: string[];
  type: string;
...
export class GoogleTrendsExploreTool extends BaseTool {
  ...
  async handle(params: GoogleTrendsExploreParams): Promise<any> {
    try {
