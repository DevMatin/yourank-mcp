...
interface GoogleAdsSearchVolumeTaskGetParams {
  task_id: string;
}
...
export class GoogleAdsSearchVolumeTaskGetTool extends BaseTool {
  ...
  async handle(params: GoogleAdsSearchVolumeTaskGetParams): Promise<any> {
    try {
      const response = await this.client.makeRequest('/v3/keywords_data/google_ads/search_volume/task_get/${params.task_id}', 'POST', 'GET');
