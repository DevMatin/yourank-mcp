...
interface GoogleAdsSearchVolumeLiveParams {
  location_name?: string | null;
  language_code?: string | null;
  keywords: string[];
}

export class GoogleAdsSearchVolumeLiveTool extends BaseTool {
  constructor(private client: DataForSEOClient) {
    super(client);
  }

  getName(): string {
    return 'keywords_data_google_ads_search_volume_live';
  }

  getDescription(): string {
    return 'Get search volume data for keywords from Google Ads in real-time (Live method)';
  }
...
optional field`),
  ...
  async handle(params: GoogleAdsSearchVolumeLiveParams): Promise<any> {
    try {
