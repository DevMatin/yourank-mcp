...
interface GoogleAdsKeywordsForSiteTaskPostParams {
  target: string;
...
interface GoogleAdsKeywordsForSiteTaskPostRequest {
  target: string;
  location_name?: string;
  language_code?: string;
  postback_url?: string;
...
export class GoogleAdsKeywordsForSiteTaskPostTool extends BaseTool {
  ...
  async handle(params: GoogleAdsKeywordsForSiteTaskPostParams): Promise<any> {
    try {
      const requestData: GoogleAdsKeywordsForSiteTaskPostRequest = {
