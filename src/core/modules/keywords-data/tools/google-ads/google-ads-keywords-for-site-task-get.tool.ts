...
interface GoogleAdsKeywordsForSiteTaskGetParams {
  task_id: string;
}
...
export class GoogleAdsKeywordsForSiteTaskGetTool extends BaseTool {
  ...
  async handle(params: GoogleAdsKeywordsForSiteTaskGetParams): Promise<any> {
    try {
      const response = await this.client.makeRequest('/v3/keywords_data/google_ads/keywords_for_site/task_get/${params.task_id}', 'POST', 'GET');
