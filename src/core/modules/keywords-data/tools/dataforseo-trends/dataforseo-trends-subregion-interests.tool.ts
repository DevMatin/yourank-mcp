...
interface SubregionInterestsParams {
  location_name?: string | null;
  keywords: string[];
  type?: 'web' | 'news' | 'ecommerce';
  date_from?: string;
  date_to?: string;
  time_range?: 'past_4_hours' | 'past_day' | 'past_7_days' | 'past_30_days' | 'past_90_days' | 'past_12_months' | 'past_5_years';
}
...
export class DataForSeoTrendsSubregionInterestsTool extends BaseTool {
  ...
  async handle(params: SubregionInterestsParams): Promise<any> {
    try {
      const response = await this.client.makeRequest('/v3/keywords_data/dataforseo_trends/subregion_interests/live', 'POST', [{
