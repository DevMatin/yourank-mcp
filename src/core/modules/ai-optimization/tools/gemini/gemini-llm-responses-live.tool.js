...
export class GeminiLlmResponsesLiveTool extends BaseTool {
  constructor(client) {
    super(client);
    this.client = client;
  }
  ...
  getName() {
    return 'ai_optimization_gemini_llm_responses_live';
  }

  getDescription() {
    return 'Live Gemini LLM Responses endpoint allows you to retrieve structured responses from a specific Gemini AI model';
  }

  getParams() {
    return {
      target: { type: "string", description: "Target parameter for the request" },
      model: { type: "string", description: "Gemini model to use", optional: true },
  ...
  async handle(params) {
    try {
      ...
      const response = await this.client.makeRequest('/v3/ai_optimization/gemini/llm_responses/live', 'POST', [requestData]);
