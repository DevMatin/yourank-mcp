...
export class GooglePlayDataInfoLiveAdvancedTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'google_play_data_info_live_advanced';
  }

  getDescription(): string {
    return 'Get detailed information about a Google Play application including ID, icon, description, reviews count, rating, number of installs, images, and other data. Requires app_id parameter.';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/app_data/google/app_info/task_post', 'POST', [requestData]);
