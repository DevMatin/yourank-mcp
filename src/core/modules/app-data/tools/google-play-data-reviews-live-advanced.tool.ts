...
export class GooglePlayDataReviewsLiveAdvancedTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'google_play_data_reviews_live_advanced';
  }

  getDescription(): string {
    return 'Get reviews published on Google Play for a specific app including review ratings, content, user profile info, publication dates, and more. Requires app_id parameter.';
  }

  getParams(): z.ZodRawShape {
    return {
      app_id: z.string().describe("Target application ID on Google Play (e.g., 'com.android.chrome')"),
      ...
      sort_by: z.enum(['newest', 'most_relevant']).optional().describe("Sort order for reviews"),
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/app_data/google/app_reviews/task_post', 'POST', [requestData]);
