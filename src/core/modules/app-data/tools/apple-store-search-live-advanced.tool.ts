...
export class AppleStoreSearchLiveAdvancedTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'apple_store_search_live_advanced';
  }

  getDescription(): string {
    return 'Get a list of apps ranking on the Apple App Store for a specified keyword with additional information like ID, icon, reviews count, rating, price, and other data. Results are specific to the keyword, language, and location parameters.';
  }
