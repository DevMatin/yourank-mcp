...
export class AppleStoreDataAppSearchesLiveAdvancedTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'apple_store_data_app_searches_live_advanced';
  }

  getDescription(): string {
    return 'Get a list of apps ranking on Apple App Store for a specified keyword with detailed information including ID, icon, reviews count, rating, price, and other metadata. This is an alias for apple_store_search_live_advanced.';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/app_data/apple/app_searches/task_post', 'POST', [requestData]);
