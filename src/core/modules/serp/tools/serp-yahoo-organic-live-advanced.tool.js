...
export class SerpYahooOrganicLiveAdvancedTool extends BaseTool {
  constructor(dataForSEOClient) {
    super(dataForSEOClient);
  }

  getName() {
    return 'serp_yahoo_organic_live_advanced';
  }

  getDescription() {
    return 'Get Yahoo Organic search results with advanced features';
  }
  ...
  async handle(params) {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/serp/yahoo/organic/live/advanced', 'POST', [requestData]);
