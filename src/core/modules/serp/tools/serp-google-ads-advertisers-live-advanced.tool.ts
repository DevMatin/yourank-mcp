...
export class SerpGoogleAdsAdvertisersLiveAdvancedTool extends BaseTool {
  ...
  getDescription(): string {
    return 'Get Google Ads Advertisers results with advanced features';
  }

  getParams(): z.ZodRawShape {
    return {
      keyword: z.string().describe("Search keyword for advertisers"),
      ...
      depth: z.number().min(1).max(700).optional().default(20).describe("Number of advertiser results to retrieve")
    };
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/serp/google/ads_advertisers/live/advanced', 'POST', [requestData]);
