...
export class GoogleAiModeTaskPostTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'google_ai_mode_task_post';
  }

  getDescription(): string {
    return 'Post Google AI Mode SERP task';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/serp/google/ai_mode/task_post', 'POST', [requestData]);
