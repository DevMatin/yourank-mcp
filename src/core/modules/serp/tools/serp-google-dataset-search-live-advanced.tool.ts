...
export class SerpGoogleDatasetSearchLiveAdvancedTool extends BaseTool {
  ...
  getDescription(): string {
    return 'Get Google Dataset Search results with advanced features';
  }
  ...
  getParams(): z.ZodRawShape {
    return {
      ...
      depth: z.number().min(1).max(700).optional().default(20).describe("Number of dataset results to retrieve")
    };
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/serp/google/dataset_search/live/advanced', 'POST', [requestData]);
