...
export class SerpGoogleOrganicLiveHtmlTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'serp_google_organic_live_html';
  }

  getDescription(): string {
    return 'Get Google organic search results as HTML in real-time';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/serp/google/organic/live/html', 'POST', [requestData]);
