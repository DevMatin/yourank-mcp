...
export class DatabasesGoogleSerpsAdvancedTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'databases_google_serps_advanced';
  }

  getDescription(): string {
    return 'Access Google Advanced SERP Database containing all kinds of SERP elements including organic and paid results, featured snippets, knowledge graphs, top stories, and more with Page Rank and Domain Rank metrics';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      console.log('🔍 Databases Google SERPs Advanced Request:', JSON.stringify(params, null, 2));
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/databases/google/serps_advanced', 'POST', [requestData]);
