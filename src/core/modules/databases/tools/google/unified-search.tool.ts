...
export class DatabasesGoogleUnifiedSearchTool extends BaseTool {
  constructor(dataForSEOClient: DataForSEOClient) {
    super(dataForSEOClient);
  }

  getName(): string {
    return 'databases_google_unified_search';
  }

  getDescription(): string {
    return 'Access Google Unified Search Database combining Advanced Google SERP Database and Google Keyword Database with millions of SERPs and billions of search terms enriched with Google Ads data';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      console.log('🔍 Databases Google Unified Search Request:', JSON.stringify(params, null, 2));
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/databases/google/unified_search', 'POST', [requestData]);
