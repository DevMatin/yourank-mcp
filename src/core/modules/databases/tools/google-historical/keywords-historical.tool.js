...
export class DatabasesGoogleHistoricalKeywordsTool extends BaseTool {
  constructor(dataForSEOClient) {
    super(dataForSEOClient);
  }

  getName() {
    return 'databases_google_historical_keywords';
  }

  getDescription() {
    return 'Access Google Historical Keywords Database providing billions of keywords enriched with historical keyword data since the beginning of 2019 including search volume trends and performance metrics';
  }
  ...
  getParams() {
    return {
      ...
      date_from: z.string().optional().describe('Start date for historical data (YYYY-MM-DD format, minimum: 2019-01-01)'),
  ...
  async handle(params) {
    try {
      console.log('🔍 Databases Google Historical Keywords Request:', JSON.stringify(params, null, 2));
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/databases/google_historical/keywords', 'POST', [requestData]);
