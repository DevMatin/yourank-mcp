...
export class DatabasesGooglePlayListingsTool extends BaseTool {
  constructor(dataForSEOClient) {
    super(dataForSEOClient);
  }

  getName() {
    return 'databases_google_play_listings';
  }

  getDescription() {
    return 'Access Google Play Listings Database encompassing over a million Google Play applications with related app data including app ID, icon, reviews count, rating, images, number of installs, price, and more';
  }
  ...
  getParams() {
    return {
      ...
      installs_min: z.number().optional().describe('Minimum number of installs filter'),
      installs_max: z.number().optional().describe('Maximum number of installs filter'),
  ...
  async handle(params) {
    try {
      console.log('🔍 Databases Google Play Listings Request:', JSON.stringify(params, null, 2));
      ...
      if (params.installs_min) requestData.installs_min = params.installs_min;
      if (params.installs_max) requestData.installs_max = params.installs_max;
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/databases/google_play/listings', 'POST', [requestData]);
