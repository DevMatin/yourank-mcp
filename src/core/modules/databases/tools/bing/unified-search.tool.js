...
export class DatabasesBingUnifiedSearchTool extends BaseTool {
  constructor(dataForSEOClient) {
    super(dataForSEOClient);
  }

  getName() {
    return 'databases_bing_unified_search';
  }

  getDescription() {
    return 'Access Full Bing Database combining Advanced Bing SERP Database and Bing Keyword Database with millions of Bing search result pages and search terms enriched with Bing Ads data';
  }
  ...
  async handle(params) {
    try {
      console.log('🔍 Databases Bing Unified Search Request:', JSON.stringify(params, null, 2));
      ...
      const response = await this.dataForSEOClient.makeRequest('/v3/databases/bing/unified_search', 'POST', [requestData]);
