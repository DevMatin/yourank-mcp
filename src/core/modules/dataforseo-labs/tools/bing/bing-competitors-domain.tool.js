...
export class BingCompetitorsDomainTool extends BaseTool {
  constructor(client) {
    super(client);
    this.client = client;
  }
  ...
  getName() {
    return 'bing_competitors_domain';
  }

  getDescription() {
    return 'Provides you with a full overview of ranking and traffic data of the competitor domains from organic and paid search';
  }
  ...
  getParams() {
    return {
      ...
      limit: z.number().optional().describe("Maximum number of returned competitors. Default value: 100, maximum value: 1000"),
  ...
  async handle(params) {
    try {
      ...
      const response = await this.client.makeRequest('/v3/dataforseo_labs/bing/competitors_domain/live', 'POST', [requestData]);
