...
export class AmazonRankedKeywordsTool extends BaseTool {
  ...
  getName() {
    return 'amazon_ranked_keywords';
  }

  getDescription() {
    return 'Provides you with a list of keywords the target product ranks for on Amazon';
  }

  getParams() {
    return {
      asin: z.string().describe("ASIN. Required field"),
  ...
  async handle(params) {
    try {
      ...
      const response = await this.client.makeRequest('/v3/dataforseo_labs/amazon/ranked_keywords/live', 'POST', [requestData]);
