...
export class AmazonBulkSearchVolumeTool extends BaseTool {
  ...
  getName() {
    return 'amazon_bulk_search_volume';
  }

  getDescription() {
    return 'Provides you with search volume values for a maximum of 1,000 keywords in one API request';
  }
  ...
  async handle(params) {
    try {
      ...
      const response = await this.client.makeRequest('/v3/dataforseo_labs/amazon/bulk_search_volume/live', 'POST', [requestData]);
