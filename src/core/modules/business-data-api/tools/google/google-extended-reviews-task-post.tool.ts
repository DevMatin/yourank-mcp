...
export class GoogleExtendedReviewsTaskPostTool extends BaseTool {
  ...
  getName(): string {
    return 'google_extended_reviews_task_post';
  }

  getDescription(): string {
    return 'This endpoint provides results from the "Reviews" element of Google SERPs, including not only Google user reviews but also reviews from other reputable sources (e.g., TripAdvisor, Yelp, Trustpilot). The results are specific to the selected location and language parameters.';
  }
  ...
  async handle(params: any) {
    ...
    return await this.dataForSEOClient.makeRequest('/v3/business_data/google/extended_reviews/task_post', 'POST', requestData);
  }
}
