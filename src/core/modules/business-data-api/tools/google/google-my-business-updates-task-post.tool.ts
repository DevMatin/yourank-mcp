...
export class GoogleMyBusinessUpdatesTaskPostTool extends BaseTool {
  ...
  getName(): string {
    return 'google_my_business_updates_task_post';
  }

  getDescription(): string {
    return 'This endpoints provides the latest updates of a specific business entity from Google SERP. The provided results are specific to the selected location and language settings.';
  }
  ...
  async handle(params: any) {
    ...
    return await this.dataForSEOClient.makeRequest('/v3/business_data/google/my_business_updates/task_post', 'POST', requestData);
  }
}
